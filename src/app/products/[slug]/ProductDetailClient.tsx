...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => { addRecentlyViewed(product); }, [product.id]); // eslint-disable-line react-hooks/exhaustive-deps

  const recentOther = recentlyViewed.filter((p) => p.id !== product.id).slice(0, 4);
        ...
        <div>
          ...
          <div className="mb-6">
            {product.inStock ? (
              ...
              <div>
                <span className="inline-flex items-center gap-2 text-red-600 font-medium text-sm bg-red-50 px-3 py-1.5 rounded-full mb-3">
                  Out of Stock
                </span>
...
function NotifyMeForm({ productId }: { productId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/notify-restock', {
        ...
        body: JSON.stringify({ productId, contact: contact.trim(), type }),
      });
  ...
  if (status === 'done') {
    return (
      <p className="text-sm text-teal-700 bg-teal-50 px-4 py-2.5 rounded-xl font-medium">
    ...
    <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition-colors ${type === 'phone' ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200'}`}
        >
          Phone / WhatsApp
        </button>
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition-colors ${type === 'email' ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200'}`}
        >
          Email
        </button>
      </div>
      <div className="flex gap-2">
        <input
          ...
          placeholder={type === 'phone' ? '+260 97X XXX XXX' : 'you@example.com'}
        ...
        <button
          ...
          className="bg-gray-900 hover:bg-gray-700 text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors disabled:bg-gray-300"
        >
          {status === 'loading' ? '…' : 'Notify me'}
        </button>
      </div>
      {status === 'error' && <p className="text-red-500 text-xs">Failed to save. Please try again.</p>}
    </form>
  );
}
